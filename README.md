...
- **Formula** (`js/app.js`, `renderFormulaSearch`, data in `js/formulas.js`) — not a calculator itself: a growing reference list (20 formulas so far) for topics that don't have a full calculator yet. Shows every entry on open — browsing is as useful as searching at this size — and narrows as you type. Searches topic, formula and note text. A card links straight to the real calculator once one is built for that topic — the two lists are joined only by tool name at render time
...
js/formulas.js       Formula reference data for the Formula screen — separate from js/data.js, joined to it by tool name
